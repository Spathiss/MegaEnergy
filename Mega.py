    ...
    def __init__(self, parent, title, options, selected_set, callback, is_package=False):
        ...
        if is_package:
            ...
            for i, (txt, clr, kw) in enumerate(tags):
                btn = ctk.CTkButton(smart_f, text=txt, fg_color=clr, text_color="black" if i==1 else "white", 
            ...
            for i, (txt, clr, kw) in enumerate(tags):
                btn = ctk.CTkButton(smart_f, text=txt, fg_color="#444", border_width=1, border_color=clr,
                                    width=80, height=24, command=lambda k=kw: self.mass_toggle(k, False))
        ...
        for option in options:
            ...
            cb = ctk.CTkCheckBox(self.scroll, text=str(option), variable=var, 
                                 command=lambda o=option, v=var: self.toggle_option(o, v))
...
class MegaEnergyCRM(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("MEGA ENERGY CRM - v60.0 FINAL")
        ...
        ctk.CTkButton(self.nav_frame, text="Επόμενη ▶", command=self.next_page, width=100).pack(side="right", padx=20)
    ...
    def start_load_thread(self):
        """Ξεκινάει το Threading για να μην παγώσει το GUI"""
    ...
    def threaded_load(self, path):
        """Η βαριά δουλειά που τρέχει στο Thread"""
        try:
            ...
            self.after(0, lambda: messagebox.showerror("Error", f"Σφάλμα: {str(e)}"))
            self.after(0, self.reset_load_button)

    def finalize_load(self, df):
        """Ενημέρωση των δεδομένων στο GUI"""
        self.df = df
        ...
        messagebox.showinfo("Success", "Επιτυχής φόρτωση και επεξεργασία!")
    ...
    def apply_filters(self):
        ...
        if is_blue_active:
            self.expiry_frame.pack(after=self.entry_end, fill="x", pady=5)
        else:
        ...
        try:
            ...
            if len(s) == 8: 
                f = f[f['Ημ. Δημιουργίας Συμβολαίου'] >= pd.to_datetime(s, format='%d%m%Y')]
            if len(e) == 8:
                f = f[f['Ημ. Δημιουργίας Συμβολαίου'] <= pd.to_datetime(e, format='%d%m%Y')]
            
            if is_blue_active:
                es = self.expiry_start.get().replace("/", "").strip()
                ee = self.expiry_end.get().replace("/", "").strip()
                if len(es) == 8:
                    f = f[f['expiry_dt'] >= pd.to_datetime(es, format='%d%m%Y')]
                if len(ee) == 8:
                    f = f[f['expiry_dt'] <= pd.to_datetime(ee, format='%d%m%Y')]
        except: pass
    ...
    def create_card(self, row):
        ...
        ctk.CTkLabel(card, text=f"{'💎 ' if is_max else ('🔵 ' if is_blue else '')}{name}", 
                     font=("Arial", 14, "bold"), 
                     text_color="#3b8ed0" if is_blue else "white").pack(anchor="w", padx=15, pady=5)
        ...
        for lbl, col in self.available_info.items():
            ...
            if self.show_vars.get(col) and self.show_vars[col].get():
                ...
                if lbl == "Λήξη Συμβολαίου" and is_blue:
                    current_color = "#3b8ed0"
                    current_font = ("Arial", 10, "bold")
                
                l = ctk.CTkLabel(info_f, text=f"{lbl}: {val}", font=current_font, text_color=current_color)
                l.grid(row=r, column=c, padx=10, pady=1, sticky="w")
                l.bind("<Button-3>", lambda e, v=val: self.copy_to_clipboard(v))
                c += 1
                if c > 2: c = 0; r += 1
    ...
    def export_to_excel(self):
        ...
        if path:
            cols_to_export = []
            name_col = 'Όνομα Πελάτη' if 'Όνομα Πελάτη' in self.filtered_df.columns else 'Επωνυμία / Ονοματεπώνυμο'
            cols_to_export.append(name_col)
            
            for lbl, col_id in self.available_info.items():
                if self.show_vars[col_id].get() and col_id in self.filtered_df.columns:
                    cols_to_export.append(col_id)
            
            final_df = self.filtered_df[list(dict.fromkeys(cols_to_export))].copy()
    ...
    def render_page(self):
        ...
        self.status_label.configure(text=f"Βρέθηκαν {total} εγγραφές")
...
if __name__ == "__main__":
    mp.freeze_support() # Απαραίτητο για το multiprocessing στα Windows
    app = MegaEnergyCRM()
    app.mainloop()